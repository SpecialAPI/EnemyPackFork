...
{
    public class SachielFleetingPassiveAbility : BasePassiveAbilitySO
    {
        ...
        public override void TriggerPassive(object sender, object args)
        {
            ...
            unit.TryGetStoredData(fleeting_USD, out var holder2);
            int mainData = holder2.m_MainData;
            mainData++;
            if (mainData >= _turnsBeforeFleeting)
            {
                ...
                unit.UnitWillFlee();
                LoadedDBsHandler.InfoHolder.Run.inGameData.SetBoolData("SachielEntered", false);
                CombatManager.Instance.AddSubAction(new FleetingUnitAction(unit.ID, unit.IsUnitCharacter));
            }

            holder2.m_MainData = mainData;
        }

        public override void OnPassiveConnected(IUnit unit)
        {
            unit.TryGetStoredData(fleeting_USD, out var holder);
            holder.m_MainData = 0;
        }
