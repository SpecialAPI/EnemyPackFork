...
{
    public class FieldEffectSeraphimTwo_Apply_Effect : EffectSO
    {
        [Header("Field")]
        ...
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            ...
            {
                if (targets[i].HasUnit && targets[i].Unit.SimpleGetStoredValue("SeraphimFire") > 0) 
                { 
                exitAmount += ApplyFieldEffect(stats, targets[i], entryVariable);
                targets[i].Unit.SimpleSetStoredValue("SeraphimFire", 0);
                }
            }
